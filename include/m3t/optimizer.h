...
class Optimizer {
 public:
  ...
  void set_phaseNum(int phaseNum);

  // Main methods
  bool CalculateConsistentPoses();
  bool myCalculateConsistentPoses();
  bool CalculateOptimization(int iteration, int corr_iteration,
                             int opt_iteration, int optimizer_iteration);

  //My methods
  bool notifyOptimizer();
 ...
 private:
  ...
  bool myUpdatePoses(const Eigen::VectorXf &theta);
  static bool myUpdatePoses(const std::shared_ptr<Link> &link_ptr,
                          const std::shared_ptr<Link> &parent_link_ptr,
                          const Eigen::VectorXf &theta);
