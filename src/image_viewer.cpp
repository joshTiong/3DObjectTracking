...
bool ImageColorViewer::UpdateViewer(int save_index, int viewer_iteration) {
  if (!set_up_) {
    std::cerr << "Set up image color viewer " << name_ << " first" << std::endl;
    return false;
  }
  DisplayAndSaveImage(save_index, color_camera_ptr_->image());
  //std::cout << "ImageColorViewer::UpdateViewer(int save_index) " << std::endl;
  return true;
}
...
bool ImageDepthViewer::UpdateViewer(int save_index, int viewer_iteration) {
  if (!set_up_) {
