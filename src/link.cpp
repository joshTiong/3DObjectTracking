...
bool Link::CalculateJacobian(const std::shared_ptr<Link> &parent_link_ptr) {
  ...
  auto dtheta_body_dtheta_joint{Adjoint(joint2body_pose)}; //Matrix<6,6>
...
bool Link::CalculateGradientAndHessian() {
  int loop = 0;
  Eigen::Matrix<float, 6, 1> gradient1;
  Eigen::Matrix<float, 6, 1> gradient2;
...
bool Link::myUpdatePoses(const std::shared_ptr<Link> &parent_link_ptr,
                       Eigen::VectorXf theta) {
  if (!IsSetup(true)) return false;
