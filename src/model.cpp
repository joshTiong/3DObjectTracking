...
bool Model::SetUpRenderer(
  ...
  if (z_min < sphere_radius_ * kMinimumClipSpaceRatio) {
    std::cerr << "z_min = " << z_min << " calculated for "
              ...
              << " is too small for model with sphere radius (here is 1st one) = "
              << sphere_radius_ << "!\n";
    //std::cout << "\nsphere_radius_ * kMinimumClipSpaceRatio = " << sphere_radius_ * kMinimumClipSpaceRatio << std::endl;
    //std::cout << "\nz_min = " << z_min << std::endl;
    //std::cout << "\nmaximum_body_diameter  = " << maximum_body_diameter  << std::endl;
    return false;
  }
  ...
  for (auto &body_ptr : body_ptrs) {
    ...
    if (z_min < sphere_radius_ * kMinimumClipSpaceRatio) {
      std::cerr << "z_min = " << z_min << " calculated for " << body_ptr->name()
                << " is too small for model with sphere radius (2nd one) = "
                << sphere_radius_ << "!\n";
      return false;
    }
...
void Model::RenderAndFetchNormalImage(
  ...
  renderer_ptr->StartRendering(viewer_iteration );
