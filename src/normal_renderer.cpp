...
bool NormalRendererCore::StartRendering(
    ...
    const Transform3fA &world2camera_pose, int viewer_iteration) {
  //std::cout << "NormalRendererCore::StartRendering" << std::endl;
  ...
  for (const auto &render_data_body :
    ...
    //std::cout << "NormalRendererCore::StartRendering >>> viewer_iteration = " << viewer_iteration << std::endl;
    //std::cout << "NormalRendererCore::StartRendering >>> &projection_matrix = \n" << projection_matrix << std::endl;
    //std::cout << "NormalRendererCore::StartRendering >>> &world2camera_pose = \n" << world2camera_pose.matrix() << std::endl;

    // Choosing pose to be rendered
    Transform3fA trans_without_projection;//{world2camera_pose * render_data_body.body_ptr->geometry2world1_pose()};
    
    if (viewer_iteration == 1){
        trans_without_projection = world2camera_pose * render_data_body.body_ptr->geometry2world2_pose(); //render_data_body.body_ptr->geometry2world_pose()
        //std::cout << "world2camera_pose = \n" << world2camera_pose.matrix() << std::endl;
    } 
    else {
        trans_without_projection = world2camera_pose * render_data_body.body_ptr->geometry2world1_pose(); //render_data_body.body_ptr->geometry2world_pose()      
        //std::cout << "world2camera_pose = \n" << world2camera_pose.matrix() << std::endl;
    }

    Eigen::Matrix4f trans{projection_matrix * trans_without_projection.matrix()};
    //Eigen::Matrix4f trans{trans_without_projection.matrix()};
    Eigen::Matrix3f rot{trans_without_projection.rotation().matrix()}; 
    //std::cout << "NormalRendererCore::StartRendering >>> trans_without_projection = \n" << trans_without_projection.matrix() << std::endl;
    ...
    //loc = glGetUniformLocation(shader_program_, "tran");
    //glUniformMatrix3fv(loc, 1, GL_FALSE, rot.data());
...
bool FullNormalRenderer::SetUp() {
  std::cout << "FullNormalRenderer::SetUp()" << std::endl;
...
void FullNormalRenderer::set_projectionMatrix_camera1(Eigen::Matrix4f projection_matrix_camera1){
  projection_matrix_camera1_ = projection_matrix_camera1;
  std::cout << "FullNormalRenderer::set_projectionMatrix_camera1" << std::endl;
}

void FullNormalRenderer::set_projectionMatrix_camera2(Eigen::Matrix4f projection_matrix_camera2){
  projection_matrix_camera1_ = projection_matrix_camera2;
}

bool FullNormalRenderer::StartRendering(int viewer_iteration) {
  const std::lock_guard<std::mutex> lock{mutex_};
  if (!set_up_) {
    std::cerr << "Set up renderer " << name_ << " first" << std::endl;
    return false;
  }
  //std::cout << "FullNormalRenderer::StartRendering()" << std::endl;
  //std::cout << "FullNormalRenderer::StartRendering() >>> projection_matrix_camera1_ = \n" << projection_matrix_camera1_ << std::endl;
  //std::cout << "FullNormalRenderer::StartRendering() >>> projection_matrix_camera2_ = \n" << projection_matrix_camera2_ << std::endl;
  //std::cout << "FullNormalRenderer::StartRendering() >>> projection_matrix_ = \n" << projection_matrix_ << std::endl;
  return core_.StartRendering(projection_matrix_, world2camera_pose_, viewer_iteration);
}
...
const Eigen::Matrix4f &FullNormalRenderer::getProjectionMatrix() const {
...
bool FocusedNormalRenderer::SetUp() {
  std::cout << "FocusedNormalRenderer::SetUp()" << std::endl;
  const std::lock_guard<std::mutex> lock{mutex_};
  set_up_ = false;
  if (!metafile_path_.empty())
    if (!LoadMetaData()) return false;
...
bool FocusedNormalRenderer::StartRendering(int viewer_iteration) {
  std::cout << "FocusedNormalRenderer::StartRendering(int viewer_iteration)" << std::endl;
  const std::lock_guard<std::mutex> lock{mutex_};
  if (!set_up_) {
    std::cerr << "Set up renderer " << name_ << " first" << std::endl;
    return false;
  ...
  CalculateProjectionMatrix();
  //std::cout << "FocusedNormalRenderer::StartRendering()" << std::endl;
  return core_.StartRendering(projection_matrix_, world2camera_pose_, viewer_iteration);
}
