...
void Optimizer::set_phaseNum(int phaseNum) { phaseNum_ = phaseNum; }
...
bool Optimizer::myCalculateConsistentPoses() {
  if (!set_up_) {
    std::cerr << "Set up optimizer " << name_ << " first" << std::endl;
    return false;
  }
  return myUpdatePoses(Eigen::VectorXf::Zero(degrees_of_freedom_));
}
...
bool Optimizer::CalculateOptimization(int iteration, int corr_iteration,
  ...
  std::cout << "iteration[" << iteration << "], corr_iteration["
            << corr_iteration << "], opt_iteration[" << opt_iteration << "] \n"
            << std::endl;

  if (iteration >= 30) {
    const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision,
                                           Eigen::DontAlignCols, ",", "\n");
    std::ofstream file("../../output/theta_phase" + std::to_string(phaseNum_) +
                       "_imgIter" + std::to_string(iteration) + "_corrIter" +
                       std::to_string(corr_iteration) + "_optIter" +
                       std::to_string(opt_iteration) + "_optimizerIter" +
                       std::to_string(optimizer_iteration) + ".csv");
    if (file.is_open()) {
      file << theta.format(CSVFormat);
      file.close();
    }
  }
...
// My method
bool Optimizer::myUpdatePoses(const Eigen::VectorXf &theta) {
  return myUpdatePoses(root_link_ptr_, nullptr, theta);
}

bool Optimizer::myUpdatePoses(const std::shared_ptr<Link> &link_ptr,
                              const std::shared_ptr<Link> &parent_link_ptr,
                              const Eigen::VectorXf &theta) {
  if (!link_ptr->myUpdatePoses(parent_link_ptr, theta)) return false;
  // std::cout << "-----Optimizer::myUpdatePoses
  // !link_ptr->UpdatePoses(parent_link_ptr, theta)" << std::endl;
  for (auto &child_link_ptr : link_ptr->child_link_ptrs())
    if (!myUpdatePoses(child_link_ptr, link_ptr, theta)) return false;
  // std::cout << "-----Optimizer::myUpdatePoses !UpdatePoses(child_link_ptr,
  // link_ptr, theta)" << std::endl;
  return true;
}
