...
void RegionModality::set_phaseNum(
    int phaseNum) {
  phaseNum_ = phaseNum;
}
...
bool RegionModality::CalculateCorrespondences(int iteration,
  ...
  // [Joshua] edited myGetClosestView() is called here
  region_model_ptr_->myGetClosestView(body2camera_pose_, &view, camera22camera1_pose_);
...
bool RegionModality::CalculateGradientAndHessian(int iteration,
                                                 int corr_iteration,
                                                 int opt_iteration, int modality_iteration) {
  if (!IsSetup()) return false;
  ...
  hessian_.setZero();
  int loop = 0;
  Eigen::MatrixXf delta_cs_vec(data_lines_.size(),1);
  Eigen::MatrixXf delta_r_vec(data_lines_.size(),1);
  ...
  for (auto &data_line : data_lines_) {
    ...
    float delta_cs = (data_line.normal_u * (xfu_z + ppu_ - data_line.center_u) +
                      ...
                      data_line.delta_r) *
                     data_line.normal_component_to_scale; //d_s

    // [Joshua] Extract delta_cs and delta_r for error analysis
    delta_cs_vec(loop,0) = delta_cs;
    delta_r_vec(loop,0) = data_line.delta_r;
    ...
    if (opt_iteration < n_global_iterations_) {
      ...
      float dloglikelihood_ddelta_cs_min = 999999;
      if (dloglikelihood_ddelta_cs < dloglikelihood_ddelta_cs_min){
        dloglikelihood_ddelta_cs_min = dloglikelihood_ddelta_cs;
      } 
    }

    // Calculate first order derivative of delta_cs with respect to theta
    //ddelta_cs_dcenter = dd_si_dcXi
  ...
  // [Joshua] Output gradient
  if (iteration >= 30){ 
    const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision, Eigen::DontAlignCols, ",", "\n");
    std::ofstream file("../../output/gradient_phase" + std::to_string(phaseNum_) + "_imgIter" + std::to_string(iteration) + "_corrIter" + std::to_string(corr_iteration) + "_optIter" + std::to_string(opt_iteration) + "_modalityIter" + std::to_string(modality_iteration) + ".csv");
      if(file.is_open())
      {
          file << gradient_.format(CSVFormat);
          file.close();
      }
  } 
  // [Joshua] Output delta_cs
    if (iteration >= 30){ 
    const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision, Eigen::DontAlignCols, ",", "\n");
    std::ofstream file("../../output/delta_cs_vec_phase" + std::to_string(phaseNum_) + "_imgIter" + std::to_string(iteration) + "_corrIter" + std::to_string(corr_iteration) + "_optIter" + std::to_string(opt_iteration) + "_modalityIter" + std::to_string(modality_iteration) + ".csv");
      if(file.is_open())
      {
          file << delta_cs_vec.format(CSVFormat);
          file.close();
      }
  } 
  // [Joshua] Output delta_r
    if (iteration >= 30){ 
    const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision, Eigen::DontAlignCols, ",", "\n");
    std::ofstream file("../../output/delta_r_vec_phase" + std::to_string(phaseNum_) + "_imgIter" + std::to_string(iteration) + "_corrIter" + std::to_string(corr_iteration) + "_optIter" + std::to_string(opt_iteration) + "_modalityIter" + std::to_string(modality_iteration) + ".csv");
      if(file.is_open())
      {
          file << delta_r_vec.format(CSVFormat);
          file.close();
      }
  } 
  return true;
...
bool RegionModality::CalculateResults(int iteration) {
  ...
  //Calc. average body2camera_pose_
  if (iteration == 51) {
    body2camera_pose_sum_ = body2camera_pose_;
  }
  else if (iteration > 51){
    //avg_num += 1;
    body2camera_pose_sum_.matrix() += body2camera_pose_.matrix();
    //body2camera_pose_avg_ /= (iteration-52) * (5) + (corr_iteration+1);
    body2camera_pose_avg_.matrix() = body2camera_pose_sum_.matrix()/(iteration-50);
    //std::cout << "RegionModality::CalculateCorrespondences >>> body2camera_pose_.matrix() = \n" << std::endl << body2camera_pose_.matrix() << std::endl;
    //std::cout << "RegionModality::CalculateCorrespondences >>> body2camera_pose_sum_ = \n" << std::endl << body2camera_pose_sum_.matrix() << std::endl;
    //std::cout << "RegionModality::CalculateCorrespondences >>> body2camera_pose_avg_ = \n" << std::endl << body2camera_pose_avg_.matrix() << std::endl;
  }   
  else{
    //through
  }  
...
const Transform3fA RegionModality::body2camera_pose_avg() const { return body2camera_pose_avg_; }
...
void RegionModality::PrecalculatePoseVariables() {
  ...
  body2camera_pose_nMINUS1_ =
      color_camera_ptr_->world2camera_pose() * body_ptr_->body2world_pose_nMINUS1();
  if (measure_occlusions_) {
  ...
  // [Joshua] calculation of camera22camera1_pose_
  camera22camera1_pose_ = body_ptr_->camera22camera1_pose();
}
...
void RegionModality::AddLinePixelColorsToTempHistograms(
  ...
  // [Joshua] myGetClosestView is called
  region_model_ptr_->myGetClosestView(body2camera_pose_, &view, camera22camera1_pose_);
