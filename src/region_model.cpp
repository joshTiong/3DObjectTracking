...
bool RegionModel::GetClosestView(const Transform3fA &body2camera_pose,
  ...
  Eigen::Vector3f orientation_camera1{
  ...
  for (auto &view : views_) {
    // std::cout << "RegionModel::GetClosestView >>> loop = " << loop <<
    // std::endl;
    float dot = orientation_camera1.dot(view.orientation);
    if (dot > closest_dot) {
      *closest_view = &view;
      closest_dot = dot;
      if (closest_dot > 0.98) {
        std::cout << "RegionModel::GetClosestView >>> closest_view = "
                  << closest_view << std::endl;
        std::cout << "RegionModel::GetClosestView >>> closest_dot = "
                  << closest_dot << std::endl;
      }
    }
    // loop++;
  }
...
bool RegionModel::myGetClosestView(
    const Transform3fA &body2camera_pose, const View **closest_view,
    const Transform3fA &camera22camera1_pose) const {
  if (!set_up_) {
    std::cerr << "Set up region model " << name_ << " first" << std::endl;
    return false;
  }
  ...
  Transform3fA camera12camera2_pose = camera22camera1_pose.inverse();
  Transform3fA body2camera2_pose = camera12camera2_pose * body2camera_pose;

  Eigen::Vector3f orientation_camera1{
      body2camera_pose.rotation().inverse() *
      body2camera_pose.translation().matrix().normalized()};
  Eigen::Vector3f orientation_camera2{
      body2camera2_pose.rotation().inverse() *
      body2camera2_pose.translation().matrix().normalized()};

  float closest_dot_sum = -0.1f;
  int loop = 0;
  for (auto &view1 : views_) {
    // Camera 1
    float dot1 = orientation_camera1.dot(view1.orientation);

    // Camera 2
    Eigen::Matrix4f view2_matrix = view1.matrix*camera22camera1_pose.matrix();
    Eigen::Matrix3f view2_rotation = view2_matrix.topLeftCorner<3,3>();
    Eigen::Vector3f view2_translation = view2_matrix.col(3).segment(0,3);
    Eigen::Vector3f view2_orientation = view2_matrix.matrix().col(2).segment(0, 3);
    float dot2 = std::abs(orientation_camera2.dot(view2_orientation));

    // [Joshua ] dot product of camera 1 and camera 2 are summed.
    float dot_sum = dot1 + dot2;
    if (dot_sum > closest_dot_sum) {
        *closest_view = &view1;
        closest_dot_sum = dot_sum;
    }
    loop++;
...
#pragma omp for
    for (int i = 0; i < int(views_.size()); ++i) {
      ...
      views_[i].matrix =
          camera2body_poses[i].matrix();
      views_[i].rotation =
          camera2body_poses[i].rotation();
      views_[i].translation =
          camera2body_poses[i].translation();
      //std::cout << "RegionModel::GenerateModel() >>> views_[i].matrix =\n "<< views_[i].matrix << std::endl; 
...
bool RegionModel::LoadModel() {
  ...
  for (size_t i = 0; i < n_views; i++) {
    ...
    ifs.read((char *)(tv.matrix.data()), sizeof(tv.matrix));
...
bool RegionModel::SaveModel() const {
  ...
  for (const auto &v : views_) {
    ...
    ofs.write((const char *)(v.matrix.data()), sizeof(v.matrix));
...
bool RegionModel::GeneratePointData(
  ...
  //std::cout << "RegionModel::GeneratePointData" << std::endl;
  // Compute silhouette
