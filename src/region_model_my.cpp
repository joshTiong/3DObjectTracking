...
#include <m3t/region_model_my.h>
namespace m3t {

myRegionModel::myRegionModel(const std::string &name,
...
bool myRegionModel::AddAssociatedBody(
...
bool myRegionModel::DeleteAssociatedBody(const std::string &name) {
...
void myRegionModel::ClearAssociatedBodies() {
...
bool myRegionModel::GetClosestView(const Transform3fA &body2camera_pose,
  ...
  for (auto &view : views_) {
    //std::cout << "RegionModel::GetClosestView >>> loop = " << loop << std::endl;
    float dot = orientation.dot(view.orientation); 
    if (dot > closest_dot) {
      *closest_view = &view;
      closest_dot = dot;
      if (closest_dot > 0.98) {
        //std::cout << "closest_view = " << closest_view << std::endl;
        //std::cout << "closest_dot = " << closest_dot << std::endl;
      }
...
float myRegionModel::max_contour_length() const { return max_contour_length_; }

const std::vector<std::shared_ptr<Body>> &myRegionModel::associated_body_ptrs()
    const {
  return associated_body_ptrs_;
}

const std::vector<std::shared_ptr<Body>> &myRegionModel::fixed_body_ptrs() const {
  return fixed_body_ptrs_;
}

const std::vector<std::shared_ptr<Body>> &myRegionModel::movable_body_ptrs()
    const {
  return movable_body_ptrs_;
}

const std::vector<std::shared_ptr<Body>>
    &myRegionModel::fixed_same_region_body_ptrs() const {
  return fixed_same_region_body_ptrs_;
}

const std::vector<std::shared_ptr<Body>>
    &myRegionModel::movable_same_region_body_ptrs() const {
  return movable_same_region_body_ptrs_;
}

bool myRegionModel::LoadMetaData() {
...
bool myRegionModel::GenerateModel() {
...
bool myRegionModel::SaveModel() const {
...
void myRegionModel::SaveAssociatedBodyData(std::ofstream *ofs) const {
...
bool myRegionModel::LoadAssociatedBodyData(std::ifstream *ifs) {
...
myRegionModel::AssociatedRendererGeometryPtrs
myRegionModel::GenerateSetUpAssociatedRendererGeometries(
...
bool myRegionModel::SetUpAssociatedRenderers(
...
bool myRegionModel::AddBodiesToAssociatedRenderers(
...
void myRegionModel::RenderAndFetchAssociatedImages(
    const AssociatedRendererPtrs &associated_renderer_ptrs,
...
bool myRegionModel::GeneratePointData(
...
bool myRegionModel::GenerateValidContours(
...
bool myRegionModel::IsContourPointValid(
...
cv::Point2i myRegionModel::SampleContourPointCoordinate(
...
bool myRegionModel::CalculateContourSegment(
...
Eigen::Vector2f myRegionModel::ApproximateNormalVector(
...
void myRegionModel::CalculateLineDistances(
    const FullSilhouetteRenderer &main_renderer,
    const AssociatedRendererPtrs &associated_renderer_ptrs,
...
void myRegionModel::FindClosestContourPoint(
