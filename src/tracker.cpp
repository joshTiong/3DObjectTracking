...
bool Tracker::RunTrackerProcess(int loop_cali_track, bool execute_detection, bool start_tracking,
  ...
  for (int iteration = 0; ; ++iteration) {
    //std::cout << ">>>>>>>>>>>>>>>>>>>>>>>RunTrackerProcess_iteration = " << iteration << std::endl;
...
bool Tracker::ExecuteTrackingStep(int iteration) {
  for (int corr_iteration = 0; corr_iteration < n_corr_iterations_;
       ++corr_iteration) {
    //std::cout << ">>>>>>>>>>>>>>>>>>>>>>>corr_itertion index = " << corr_iteration << std::endl;
...
bool Tracker::UpdateViewers(int iteration) {
  if (!viewer_ptrs_.empty()) {
    int viewer_iteration = 0;
    for (auto &viewer_ptr : viewer_ptrs_) {
      viewer_ptr->UpdateViewer(iteration, viewer_iteration);
      ++viewer_iteration;
    }
  ...
  //std::cout << "-----Tracker::UpdateViewers(int iteration)" << std::endl;
  return true;
}
...
bool Tracker::CalculateConsistentPoses() {
  int loop = 1;
  for (auto &optimizer_ptr : optimizer_ptrs_) {
    //std::cout << "Tracker::CalculateConsistentPoses() >>> loop = " << loop << std::endl;
    //std::cout << "Tracker::CalculateConsistentPoses() >>> &optimizer_ptr = " << &optimizer_ptr << std::endl;
    //std::cout << "Tracker::CalculateConsistentPoses() >>> optimizer_ptrs_ = " << &optimizer_ptrs_[0]  << std::endl;
    //if (&optimizer_ptr == &optimizer_ptrs_[0]){
      //if (!optimizer_ptr->CalculateConsistentPoses()) return false;
    //}
    //else if (&optimizer_ptr == &optimizer_ptrs_[1]) {
      if (!optimizer_ptr->myCalculateConsistentPoses()) return false;
    //}   
    //else{
    //  std::cout << "error at Tracker::CalculateConsistentPoses()" << std::endl;
    //  exit(0);
    //} 
    loop++;
  }
...
bool Tracker::StartModalities(int iteration) {
  ...
  for (auto &start_modality_renderer_ptr :
    ...
    if (!start_modality_renderer_ptr->StartRendering(viewer_iteration)) return false;
  }
...
bool Tracker::CalculateCorrespondences(int iteration, int corr_iteration) {
  ...
  for (auto &correspondence_renderer_ptr :
    ...
    if (!correspondence_renderer_ptr->StartRendering(viewer_iteration)) return false;
  }
...
bool Tracker::CalculateGradientAndHessian(int iteration, int corr_iteration,
  ...
  std::cout << "Tracker::CalculateGradientAndHessian >>> iteration["  << iteration << "] corr_iteration[" << corr_iteration << "] update_iteration[" << update_iteration << "]" << std::endl; 
  int modality_iteration = 0;
  for (auto &modality_ptr : tracking_modality_ptrs_) {
    std::cout << "modality[" << modality_iteration << "]" << std::endl;
    if (!modality_ptr->CalculateGradientAndHessian(iteration, corr_iteration,
                                                   update_iteration, modality_iteration))
      return false;
    modality_iteration++;
  }
  return true;
...
bool Tracker::CalculateOptimization(int iteration, int corr_iteration,
  ...
  for (auto &optimizer_ptr : tracking_optimizer_ptrs_) {
    if (!optimizer_ptr->CalculateOptimization(iteration, corr_iteration,
                                              update_iteration, optimizer_iteration))
      return false;
...
bool Tracker::CalculateResults(int iteration) {
  ...
  for (auto &results_renderer_ptr : tracking_results_renderer_ptrs_) {
    if (!results_renderer_ptr->StartRendering(viewer_iteration)) return false;
  }
...
void Tracker::AssembleDerivedObjectPtrs() {
  ...
  for (auto &viewer_ptr : viewer_ptrs_) {
    std::cout << "Tracker::AssembleDerivedObjectPtrs() >> viewer_ptr >> loop = " << loop << std::endl;
